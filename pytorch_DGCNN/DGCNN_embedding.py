...
class DGCNN(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats, latent_dim=[32, 32, 32, 1], k=30, conv1d_channels=[16, 32], conv1d_kws=[0, 5], conv1d_activation='ReLU'):
        ...
        self.conv_params.append(nn.Linear((num_node_feats + num_edge_feats)*4, latent_dim[0]))
        for i in range(1, len(latent_dim)):
            self.conv_params.append(nn.Linear(latent_dim[i-1]*4, latent_dim[i]))
    ...
    def sortpooling_embedding(self, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp, graph_sizes, node_degs):
        ...
        while lv < len(self.latent_dim):
            ...
            '''
                Modular GCN Design
            '''
            F_A_X = gnn_spmm(n2n_sp, cur_message_layer)

            F_A_A_X = gnn_spmm(n2n_sp, F_A_X)

            sqrt_normalized_cur_message_layer = cur_message_layer.div(node_degs.sqrt())
            F_A_D2_X = gnn_spmm(n2n_sp, sqrt_normalized_cur_message_layer)
            F_D2_A_D2_X = F_A_D2_X.div(node_degs.sqrt())

            F_D_A_X = F_A_X.div(node_degs)

            F = torch.cat([F_A_X, F_A_A_X, F_D_A_X, F_D2_A_D2_X], 1)  # There are four kind of propaganda funciton
            node_linear = self.conv_params[lv](F)
            cur_message_layer = torch.tanh(node_linear)
