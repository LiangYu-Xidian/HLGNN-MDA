...
def loop_dataset(g_list, classifier, sample_idxes, optimizer=None, bsize=cmd_args.batch_size,retain_result = False):
    total_loss = []
    ...
    #np.savetxt('test_labels.txt', all_targets)
    if not classifier.regression:
        all_targets = np.array(all_targets)
        fpr, tpr, _ = metrics.roc_curve(all_targets, all_scores, pos_label=1)
        auc = metrics.auc(fpr, tpr)
        pred = np.array([1 if i > math.log(0.5) else 0 for i in all_scores])
        recall = metrics.recall_score(all_targets, pred)
        precision_score = metrics.precision_score(all_targets, pred)
        f_1_score = metrics.f1_score(all_targets, pred)
        avg_loss = np.concatenate((avg_loss, [auc,recall, precision_score, f_1_score]))
    pred_target = None
    if retain_result:
        pred_target = [all_scores, all_targets]
        # np.savetxt('test_scores.txt', pred)  # output test predictions
        # np.savetxt('test_labels.txt', all_targets)
    return (avg_loss, pred_target)
